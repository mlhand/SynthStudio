...
notesPlaying = []
octave = -1 # does nothing right now
...
def playChord():
    ...
    chord = currentChord.getChordAndStrumPadHertz()[0]
    for note in chord:
        kp.createSound(note, 2)
...
def playNote(noteNum):
    chord = currentChord.getChordAndStrumPadHertz()[0] + currentChord.getChordAndStrumPadHertz()[1]
    note = chord[noteNum]
    kp.createSound(note, 2)
